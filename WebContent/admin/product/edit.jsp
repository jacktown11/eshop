...
<HTML>
	...
	<body>
		<!--  -->
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/admin?method=editProduct" method="post" enctype="multipart/form-data">
			...
			<table cellSpacing="1" cellPadding="5" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff">
						<select name="is_hot">
							<option value="1" ${product.is_hot==1?"selected":""} >是</option>
							<option value="0" ${product.is_hot==0?"selected":""} >否</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff" colspan="3">
						<select name="cid">
 							<c:forEach items="${categoryList}" var="category">
								<option value="${category.cid }" ${category.cid==product.category.cid ? "selected" : "" }>${category.cname }</option>							
							</c:forEach>
						</select>
					</td>
				</tr>
